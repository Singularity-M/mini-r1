...
def init_model(lm_config):
    ...
    if model_from == 1:
        ...
        ckp = f'./out/sft_tcm_{lm_config.dim}{moe_path}.pth'
...
if __name__ == "__main__":
    ...
    prompt_datas = [
        '上次我父亲体检时发现有糖尿病、但因为血糖在正常范围内、所以医生说自己平时多注意就好、然后我发现好几个月我父亲都没有去检测血糖、他说没必要。糖尿病是不是要定期检测血糖？',
                '使用 Keras 的 mnist 数据集，训练一个卷积神经网络识别手写数字。此外，实现一个自定义的损失函数，并使用不同的数据增强技术来提高模型的泛化能力。请提供一个报告，概述模型架构，训练期间遇到的任何挑战，以及针对不同情况（例如过度拟合）所采取的对策。最后，在测试数据上执行模型，并在混淆矩阵中可视化结果。',
                '你更擅长哪一个学科？',
                '鲁迅的《狂人日记》是如何批判封建礼教的？',
                '我咳嗽已经持续了两周，需要去医院检查吗？',
                '详细的介绍光速的物理概念。',
