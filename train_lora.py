...
from model.model_lora import ModelWithLoRA, find_all_linear_names
...
def train_epoch(epoch, writer):
    ...
    for step, (X, Y, loss_mask) in enumerate(train_loader):
        ...
        if (step + 1) % args.save_interval == 0 and (not ddp or dist.get_rank() == 0):
            ...
            ckp = f'{args.save_dir}/tcm_lora_{lm_config.dim}{moe_path}.pth'
            

            if isinstance(model, torch.nn.parallel.DistributedDataParallel):
                model.module.save_lora_weights(ckp)
            else:
                model.save_lora_weights(ckp)
...
def init_model(is_continue_pretrain=False):
    ...
    model = ModelWithLoRA(model, target_modules, lora_rank=args.lora_rank,  lora_alpha=args.lora_alpha)
    model = model.to(args.device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default="data/origin_data/待处理数据集/2_ChatMed_TCM_sft.jsonl", help="Path to training data") # 根据不同的数据确定是单论对话还是多轮对话
    ...
    parser.add_argument('--lora_rank', type=int, default=128, help='')
    parser.add_argument('--lora_alpha', type=float, default=2.0, help='')
    ...
    model, tokenizer = init_model(is_continue_pretrain=False)
